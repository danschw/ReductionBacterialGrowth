...
In this round of the experiment we are looking into the incubation time of a static plate for endpoint assay.
...
d <- read_synergy_plate(here("data/20201001_OD600.xlsx"))
  

#add meta data
# There were 2 plates with the same well layout
well.meta <- read_csv(here("data","20201001_well_meta.csv" ))
# data for when plates were read
plate.meta <- read_csv(here("data","20201001_plate_meta.csv" ))
...
d.long%>%
  ...
  filter(!vortex)%>%
  ggplot(aes(x=Time, y=OD600, color=phage, group=well))+
  geom_point()+
  geom_line()+
...
```
Looks like 4 hours shroud work well  

# Edge effect (for no-phage only)
```{r}
...
d.long%>%
  ...
  ggplot(aes(x=Time, y=OD600, color=position, group=well))+
  geom_point()+
  geom_line()+
  facet_grid(host~colony)+
  theme_cowplot()+
...
There is an edge effect, with less growth in edge.  
Interestingly, it looks as if there was a difference between the right and left sides of the plate in the edge effect: WT colony 1 (ANC1)  was able to continue growing in the edge wells. Fr the mutant on the same side there was no edge effect 
